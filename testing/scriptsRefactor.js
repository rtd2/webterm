...
// -----------------------------------------------------------------------
    ...
    grep: function (search, file) {
        ...
        if (isFile) { // there is a file, let's search it
            if (search.indexOf(0) && search.indexOf(-1) === "'" || search.indexOf(0) && search.indexOf(-1) === '"') { search = search.substr(1, -1); }
...
function checkCommand(e) {
    ...
    function commandRouter () {
        ...
        if ( command != 'youtube' ) return
        if ( helpList.hasOwnProperty(command) ) {
            helpList[command].run();
        } else { defaultErrorHandler(commandArgs[1]); }
    }
        ...
        if (commandArgs.length === 2) { // if the command entered has one argument
            if (commandArgs[1] != "-help" && commandArgs[1] != "--help" && commandArgs[1] != "-h") {
                switch (commandArgs[0]) {
                    ...
                    case "history":
                        runCommand.apply(terminal.history.arg);
                        break;
